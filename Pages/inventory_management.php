    ...
    <div class="bg-surface rounded-xl border border-outline-variant overflow-hidden flex flex-col shadow-sm">
        ...
        <form method="GET" action="inventory_management.php" class="p-3 sm:p-md border-b border-outline-variant flex flex-wrap gap-2 sm:gap-4 justify-between items-center bg-surface-bright">
            ...
            <div class="flex gap-2">
                ...
                <a href="inventory_management.php" class="p-1.5 text-on-surface-variant hover:bg-surface-container rounded border border-outline-variant transition-colors flex items-center justify-center cursor-pointer" title="Reset Filters">
            ...
            <table class="w-full text-left border-collapse min-w-[750px]">
                <thead class="bg-surface-container-low font-label-md text-xs text-on-surface-variant sticky top-0">
                    <tr>
                        ...
                        <th class="py-3 px-3 sm:px-4 border-b border-outline-variant font-semibold">Expiry (Nearest)</th>
                        <th class="py-3 px-3 sm:px-4 border-b border-outline-variant font-semibold text-right">Cost Price</th>
                    ...
                    <?php else: ?>
                        <?php foreach ($medications as $med): ?>
                            <?php
                                ...
                                $expiryStr = !empty($med['nearest_expiry']) ? date('M Y', strtotime($med['nearest_expiry'])) : 'No Active Batch';
                            ?>
                            <tr class="hover:bg-surface-container-low transition-colors group">
                                ...
                                <td class="py-2.5 px-3 sm:px-4 font-code-md text-on-surface-variant"><?php echo e($med['med_code']); ?></td>
                                ...
                                <td class="py-2.5 px-3 sm:px-4 text-right text-on-surface-variant">$<?php echo number_format((float)$med['cost_price'], 2); ?></td>
                                <td class="py-2.5 px-3 sm:px-4 text-right font-bold text-on-surface">$<?php echo number_format((float)$med['unit_price'], 2); ?></td>
    ...
    <div class="bg-surface rounded-2xl border border-outline-variant max-w-xl w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl custom-scrollbar">
        ...
        <form method="POST" action="inventory_management.php" class="space-y-4">
            ...
            <div id="new-med-fields" class="p-3 bg-surface-container-lowest rounded-xl border border-outline-variant/80 hidden space-y-3">
                ...
                <div class="grid grid-cols-2 gap-2">
                    ...
                    <div>
                        ...
                        <input name="min_stock_alert" class="w-full bg-surface border border-outline-variant rounded p-2 text-xs text-on-surface" placeholder="20" type="number" value="20">
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                    <label class="block font-semibold text-xs text-on-surface mb-1">Selling Price ($ per unit)</label>
                    <input id="modal-selling-price" name="unit_price" step="0.01" class="w-full bg-surface-container-low border border-outline-variant rounded-lg p-2 text-xs text-on-surface font-semibold" type="number" placeholder="0.00">
                </div>
            </div>
...
<script>
    ...
    function toggleNewMedFields() {
        ...
        if (select.value === '0') {
            ...
            if (selectedOpt && selectedOpt.dataset.cost !== undefined) {
                ...
                document.getElementById('modal-selling-price').value = parseFloat(selectedOpt.dataset.price).toFixed(2);
            }
        }
    ...
    function prepareRestockForMed(id, name, cost, price) {
        ...
        document.getElementById('modal-selling-price').value = parseFloat(price).toFixed(2);
