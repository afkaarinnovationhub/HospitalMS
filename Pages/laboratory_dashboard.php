...
// If legacy tab=catalog was requested, redirect to dedicated lab_catalog.php
if (!empty($_GET['tab']) && $_GET['tab'] === 'catalog') {
    safeRedirect('lab_catalog.php');
}
...
$pageTitle = 'Laboratory Worklist & Diagnostics - ' . HOSPITAL_NAME;
$headerTitle = HOSPITAL_NAME . ' - Laboratory';
...
<main class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-lg pb-6 bg-background custom-scrollbar">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-md mb-lg">
        ...
        <div class="flex flex-wrap gap-sm w-full sm:w-auto">
            <button onclick="window.location.reload();" class="flex-1 sm:flex-none px-3.5 py-2 border border-outline-variant text-on-surface font-label-md text-xs rounded-xl hover:bg-surface-container transition-colors flex items-center justify-center gap-1.5 font-semibold cursor-pointer shadow-xs">
                ...
                <div class="flex flex-wrap items-center gap-2 flex-1 max-w-2xl">
                    ...
                    <select name="panel" onchange="this.form.submit()" class="bg-surface border border-outline-variant rounded-lg py-1.5 px-3 font-body-sm text-xs text-on-surface outline-none">
                        ...
                        <?php foreach ($labCategories as $cat): ?>
                            <option value="<?php echo e($cat['name']); ?>" <?php echo $panelFilter === $cat['name'] ? 'selected' : ''; ?>><?php echo e($cat['name']); ?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <table class="w-full text-left border-collapse min-w-[900px]">
                    <thead class="bg-surface-container-low border-b border-outline-variant font-label-md text-xs text-on-surface-variant">
                        <tr>
                            ...
                            <th class="py-3 px-4 font-semibold">Diagnostic Test</th>
                    ...
                    <tbody id="lab-worklist-tbody" class="font-body-sm text-xs divide-y divide-outline-variant">
                        ...
                        <?php else: ?>
                            <?php foreach ($worklist as $order): ?>
                                <?php
                                    $isCleared = ($order['payment_status'] === 'paid' || $order['payment_status'] === 'partial');
                                    $payBadge = $isCleared 
                                ...
                                <tr class="hover:bg-surface-container-low transition-colors">
                                    ...
                                    <td class="py-3 px-4">
                                        <span class="text-[10px] px-2 py-0.5 rounded-full font-bold border capitalize <?php echo $payBadge; ?>">
                                            <?php echo $isCleared ? '✓ Paid' : '🔒 Pending Fee'; ?>
                                        </span>
                                    </td>
                                    <td class="py-3 px-4">
                                        ...
                                        <div class="flex items-center justify-end gap-1.5 flex-wrap">
                                            ...
                                            <?php if (!$isCleared): ?>
                                                <a href="billing_payments.php<?php echo !empty($order['billing_invoice_id']) ? '?invoice_id=' . (int)$order['billing_invoice_id'] : ''; ?>" 
                                                   ...
                                                   title="Lab fee unpaid. Patient must settle at Cashier before specimen collection.">
